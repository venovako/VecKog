...
{
  if (S) {
#pragma omp parallel for default(none) shared(n,RE,OU,OV,A11,A21,A12,A22,U11,U21,U12,U22,V11,V21,V12,V22,S1,S2,S)
    for (size_t i = (size_t)0u; i < n; ++i) {
      RE[i] = wdmm
        (A11[i], A21[i], A12[i], A22[i], U11[i], U21[i], U12[i], U22[i], V11[i], V21[i], V12[i], V22[i], S1[i], S2[i], S[i]);
      OU[i] = wdor(U11[i], U21[i], U12[i], U22[i]);
      OV[i] = wdor(V11[i], V21[i], V12[i], V22[i]);
    }
  }
  else {
#pragma omp parallel for default(none) shared(n,RE,OU,OV,A11,A21,A12,A22,U11,U21,U12,U22,V11,V21,V12,V22,S1,S2)
    for (size_t i = (size_t)0u; i < n; ++i) {
      RE[i] = wdmm
        (A11[i], A21[i], A12[i], A22[i], U11[i], U21[i], U12[i], U22[i], V11[i], V21[i], V12[i], V22[i], S1[i], S2[i], +0.0);
      OU[i] = wdor(U11[i], U21[i], U12[i], U22[i]);
      OV[i] = wdor(V11[i], V21[i], V12[i], V22[i]);
    }
  }
...
{
  if (S) {
#pragma omp parallel for default(none) shared(n,RE,OU,OV,A11r,A11i,A21r,A21i,A12r,A12i,A22r,A22i,U11r,U11i,U21r,U21i,U12r,U12i,U22r,U22i,V11r,V11i,V21r,V21i,V12r,V12i,V22r,V22i,S1,S2,S)
    for (size_t i = (size_t)0u; i < n; ++i) {
      RE[i] = wzmm
...
#pragma omp parallel for default(none) shared(n,RE,OU,OV,A11r,A11i,A21r,A21i,A12r,A12i,A22r,A22i,U11r,U11i,U21r,U21i,U12r,U12i,U22r,U22i,V11r,V11i,V21r,V21i,V12r,V12i,V22r,V22i,S1,S2)
    for (size_t i = (size_t)0u; i < n; ++i) {
      RE[i] = wzmm
        (A11r[i], A11i[i], A21r[i], A21i[i], A12r[i], A12i[i], A22r[i], A22i[i],
         U11r[i], U11i[i], U21r[i], U21i[i], U12r[i], U12i[i], U22r[i], U22i[i],
